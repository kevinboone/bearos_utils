...
int term_get_line (int fd_in, char *buff, int len, 
  ...
  int oldflags;
#ifdef BEAROS
  devctl (0, DC_TERM_GET_FLAGS, (int32_t)&oldflags);
#else
  oldflags = DC_TERM_FLAG_ECHO; // Nasty
#endif
  
  devctl (0, DC_TERM_SET_FLAGS, (int32_t)DC_TERM_FLAG_NOECHO);

  while (!done)
    {
    int c = terminal_get_key (fd_in);
    if (c == VK_INTR)
      {
  ...
  devctl (0, DC_TERM_SET_FLAGS, (int32_t)oldflags);
