...
{
#ifdef BEAROS
	if (terminal_get_line (STDIN_FILENO, s, INPUT_BUFFER_SIZE - 2) != TGL_OK)
          {
	  os_fatal ("Interrupted");
          }
        int l = strlen (s);
        s[l] = 10;
        s[l + 1] = 0;
...
#endif
			default:
				printf("DUMB-FROTZ: unknown escape char: %c\n", src[-1]);
				printf("Enter \\help to see the list\n");
			}
		}
	*dest = '\0';
}
	...
	for (;;) {
		...
		if (!strcmp(command, "t")) {
			...
			if (type != INPUT_LINE_CONTINUED)
				printf("DUMB-FROTZ: No input to discard\n");
			else {
			...
			printf("DUMB-FROTZ: unknown command: %s\n", s);
