...
// Amount of space to insert when using the "indent" function
#ifndef INDENT
#define INDENT "  "
#endif

// Terminal codes
// This is nasty -- these really ought to be configurable. 
...
// Handy shortcut for tests for ctrl-X 
...
typedef struct _ButeEnv 
  ...
  BOOL noindent;            // If true, do not auto-indent
  } ButeEnv;
...
/*==========================================================================
  ...
  mystrerror
  In case we want to handle error codes other than the standard ones
  (but none are defined at present).
...
static void get_console_size (ButeEnv *env) 
  {
  int lines, cols;
  term_get_size (&lines, &cols);
  if (env->lines < 0) env->lines = lines;
  if (env->cols < 0) env->cols = cols;
  }
...
==========================================================================*/
ErrCode read_file (const char *filename, uint8_t** buff, int *length)
  {
  ErrCode ret = 0;
  struct stat sb;
...
==========================================================================*/
ErrCode write_file (const char *filename, uint8_t* buff, int length)
  {
  ErrCode ret = 0;
...
==========================================================================*/
static ErrCode load_file (BUTE *ed, const char *filename) 
  {
  int length;
  char *buff;
  ErrCode ret = read_file (filename, (uint8_t**) &buff, &length);
  if (ret == 0)
    {
...
ErrCode save_file (BUTE *ed) 
  ...
  if (buff)
    ...
    ret = write_file (ed->filename, (uint8_t*) buff, total);
    free (buff);
    }
  else
    ret = ENOMEM;
...
BOOL prompt (const BUTE *ed, char *msg) 
  ...
  term_write_string (STATUS_COLOR);
  term_write_string (msg);
  term_clear_eol();

  BOOL interrupt = FALSE;
  BOOL ret = term_get_line (0, buf, 50, &interrupt);
...
int says_yes() 
  {
  int ch = term_get_key (STDIN_FILENO);
  return ch == 'y' || ch == 'Y';
  }
...
void draw_statusline(BUTE *ed) 
  ...
  sprintf (ed->env->linebuf, "%c Ln %-6dCol %-4d" , 
      ed->dirty ? '*' : ' ', ed->line + 1, 
      column (ed, ed->linepos, ed->col) + 1);
  term_write_string(ed->env->linebuf);
  term_clear_eol(); 
  term_write_string (TEXT_COLOR);
  term_show_cursor(); 
  }
...
void display_line (const BUTE *ed, int pos, int fullline) 
  ...
  term_write_buff (ed->env->linebuf, bufptr - ed->env->linebuf);
  }
...
==========================================================================*/
static void update_line (const BUTE *ed) 
  {
...
==========================================================================*/
static void newline (BUTE *ed, BOOL no_indent) 
  {
  erase_selection (ed);
...
==========================================================================*/
static void indent (BUTE *ed, char *indentation)
  {
  char *buffer, *p;
  int buflen;
...
==========================================================================*/
static void unindent (BUTE *ed, char *indentation) 
  {
...
==========================================================================*/
static void copy_selection (BUTE *ed) 
  {
  int selstart, selend;
...
==========================================================================*/
static void cut_selection (BUTE *ed) 
  {
  copy_selection(ed);
  erase_selection(ed);
  }
...
==========================================================================*/
static void paste_selection (BUTE *ed) 
  {
  erase_selection(ed);
...
==========================================================================*/
static void prompt_open_editor (BUTE *ed) 
  {
  char *filename;
  ButeEnv *env = ed->env;
...
==========================================================================*/
static void new_editor (BUTE *ed) 
  {
...
==========================================================================*/
static ErrCode save_editor (BUTE *ed) 
  {
...
==========================================================================*/
static void close_editor (BUTE *ed) 
  {
  ButeEnv *env = ed->env;
...
==========================================================================*/
static void find_text (BUTE *ed, BOOL next) 
  {
  if (!next) 
    {
    if (!prompt (ed, "Find: ")) 
...
==========================================================================*/
static BUTE *next_buffer (BUTE *ed) 
  {
...
==========================================================================*/
static void redraw_screen (BUTE *ed) 
  {
...
static void help (BUTE *ed) 
  ...
  term_set_cursor (0, 0);
  term_clear ();
  puts ("Arrow and page keys move cursor; shift+move keys select text"); 
  puts ("<back>        Delete back     <del>           Delete forward"); 
  puts ("Ctrl+<left>   Prev. word      Ctrl+<right>    Next word"); 
  puts ("Ctrl+A        Select all      Ctrl-end        Bottom of file"); 
  puts ("Ctrl+E        Switch buffers  Ctrl+F          Find text"); 
  puts ("Ctrl+G        Find next       Ctrl+L          Redraw screen"); 
  puts ("Ctrl+L        Go to line      Ctrl+N          New buffer" ); 
  puts ("Ctrl+O        Open file       Ctrl+Q          Quit" ); 
  puts ("Ctrl+R        Redo            Ctrl+S          Save" ); 
  puts ("Ctrl+Home     Top of file     Ctrl+V          Paste"); 
  puts ("Ctrl+W        Close buffer    Ctrl+X          Cut"); 
  puts ("Ctrl+Y        Copy            Ctrl+Z          Undo"); 
  puts ("In a selection, <tab> indents and shift-<tab> unindents");
  puts ("Press any key to continue...");

  term_get_key (STDIN_FILENO);
  draw_screen (ed);
  draw_full_statusline (ed);
  }
...
static void edit (BUTE *ed) 
  {
  BOOL done = FALSE;
  BOOL no_indent = ed->env->noindent;
  ...
  while (!done)  // Continue until told to stop
    ...
    int key = term_get_key (STDIN_FILENO);
...
==========================================================================*/
ButeEnv *buteenv_create (BOOL noindent, int fix_rows, int fix_cols) 
  {
